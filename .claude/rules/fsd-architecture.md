...
- **このモノレポでの扱い**: UIキットと `cn` はワークスペースの `@repo/ui`
  パッケージが担うため、アプリ側の `shared/ui`・`shared/lib` には置かない。
  複数アプリで共有しない、そのアプリ固有の汎用コードだけを `shared/` に置く。
...
他のレイヤーからインポートする場合は、必ず subpath import `#/` を使用してください。
バンドラのエイリアスではなく、アプリの `package.json` の `imports` フィールドで定義しています。

#### 設定されているエイリアス

| エイリアス | パス    | 定義場所                          |
| ---------- | ------- | --------------------------------- |
| `#/*`      | `src/*` | `apps/<app>/package.json#imports` |

ワークスペースの共有パッケージは、エイリアスではなくパッケージ名で参照します。

| インポート元            | 例                           |
| ----------------------- | ---------------------------- |
| `@repo/ui/components/*` | `@repo/ui/components/button` |
| `@repo/ui/lib/*`        | `@repo/ui/lib/utils`（`cn`） |
